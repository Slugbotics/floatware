...
`rustup git wget flex bison gperf python3 python3-pip python3-venv cmake ninja-build ccache libffi-dev libssl-dev
dfu-util libusb-1.0-0 riscv32-elf-gdb`
(Python 3.7+)
...
5. `$ ~/.cargo/bin/espup install -t esp32,esp32c3 -f export-esp.sh` (should be run inside the project directory)

6. `$ . export-esp.sh` (MUST BE RUN EVERYTIME A NEW TERMINAL IS IN USE (in theory))

TODO: The CI script manages to build the binary without doing all these steps. Figure out how. Also my IDE does too? HOW
...
`$ python3 -m esp_idf_monitor [-p <THE DEVICE PORT>] --toolchain-prefix riscv32-esp-elf- --target esp32c3
--decode-panic backtrace target/riscv32imc-esp-espidf/debug/floatware`
